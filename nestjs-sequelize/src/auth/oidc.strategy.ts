...
export const buildOpenIdClient = async () => {
  console.debug('openid_configuration', `${process.env.OAUTH2_CLIENT_PROVIDER_OIDC_ISSUER}/.well-known/openid-configuration`)
  const TrustIssuer = await Issuer.discover(`${process.env.OAUTH2_CLIENT_PROVIDER_OIDC_ISSUER}/.well-known/openid-configuration`);
  console.debug('authorization_endpoint', TrustIssuer.metadata.authorization_endpoint)
